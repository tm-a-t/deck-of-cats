...
- Starting resources, `☠️`, `Boarding Alert`, `Full Crew Discount`, `Opening Counter Prep`, `Counter Watch`, the opening route cache-claimed marker, opening route counter secured marker, bought-pirate marker, `Opening Route Muster`, `Route Sidekick`, and Cache Drill bounty marks: `0`/empty.
...
- `Route Counter Cover`: when that qualifying pre-Boarding-1 route-primary purchase secures the selected route and pending `Boarding Alert` after the purchase is above `0`, immediately reduce pending `Boarding Alert` by `1`, never below `0`.
- `Route Counter Cover` applies only to secured route-primary purchases through `Full Crew Discount`, `Full Crew Discount` coverage, `Opening Counter Prep`, or `Cache-Claimed Alarm Rush Route Counter`. It grants no extra reward when no pending Alert exists and never applies to discard-only route-primary buys, route side offers, non-counters, `Battle Test`, Boarding 2+, or purchases after Boarding 1 starts.
...
- `Cache-Claimed Alarm Rush Route Counter` does not grant `Opening Counter Prep` discount, prep `Might`, `Prepared` ship gains, `Route Starter Pass-Off`, `Cache Drill` bounty marks, Alert refunds beyond possible `Route Counter Cover`, `Full Crew Discount` coverage, doubled `Ambush Bounty`, or any other cache reward.
...
- `Boarding Trophy`, `Counter Trophy`, Alert guard plunder, Opening plunder, Ambush Bounty, and Opening Route Promotion can all stack on the same win when their own conditions are met.
...
- `Route Sidekick Report` never applies in `Battle Test`, Boarding 2+, losses, reinforcement-hand wins, ordinary side-offer buys, removed sidekicks, absent sidekicks, defeated sidekicks, or wounded sidekicks. It grants no `Counter Watch`, `Counter Edge`, `Counter Ambush`, `Ambush Bounty`, `Opening Route Promotion`, route security, Alert refund, resources, or other counter perks.

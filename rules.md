...
- `Opening Full Crew Report` never grants `Counter Watch`, never becomes `Watch Ready`, never creates `Prepared`, never gains `Opening Counter Prep` Might, never shows counter payoff text, and never receives counter ambush benefits. If `Opening Counter Prep` is also active, the non-counter purchase does not consume it.
...
- `Opening Counter Prep` is consumed by the first successful eligible `Top deck` scouted-counter purchase in that Shop. Non-counter purchases do not consume it.
...
- If `Opening Counter Prep` and `Full Crew Discount` are both present, the discount still changes the price as normal and is consumed by the first successful pirate purchase, while the prep waits until an eligible `Top deck` scouted counter is bought. If that first purchase is also an eligible counter, both shop flags are consumed by that purchase. Before Boarding 1, the prep is the `+1 💪 Might` route; after Boarding 1 has started, `Full Crew Discount` keeps its normal `Prepared` trigger.
...
- If `Full Crew Discount` is spent on a non-counter first, a later `Top deck` counter purchase in the same Shop is not `Prepared`, but it can still consume banked `Opening Counter Prep` for `+1 💪 Might`.

...
- Before Boarding 1 in regular runs, an `Alarm-Rushed Route Counter` is the selected route's primary opening counter bought while the pending `Boarding Alert` after that purchase is at least `3`, counting both Alert already pending and any same-purchase `Dockside Credit` Alert.
...
- `Alarm-Rushed Route Counter` never applies to non-primary counters, non-counters, `Battle Test`, Boarding 2+, or purchases whose post-purchase pending Alert is below `3`.

...
- During regular-run Boarding 1 only, if that successful first cache opener is the selected route's `Opening Deckhand Counter` starter and that route's primary opening counter has not yet been bought, `Route Starter Cache Prep` also gives the immediately following Shop normal `Opening Counter Prep`.
...
- `Route Starter Cache Prep` does not apply in `Battle Test`, Boarding 2+, non-cache islands, failed `Cache Drill` cases, non-counter openers, bought shop-counter openers, removed openers, wrong starters, or routes whose primary opening counter is already bought.
...
- Setup-gated opening route-primary buys report early only if that same purchase spends `Full Crew Discount`, uses `Full Crew Discount` coverage, consumes `Opening Counter Prep`, or qualifies as an `Alarm-Rushed Route Counter`; those qualifying buys go on top of the draw pile and gain `Counter Watch`. A route-primary buy that meets none of those setup gates still secures the route shop slot, but goes to discard, gains no `Counter Watch`, and gains no prep `Might`.
- Before Boarding 1 in regular runs, an `Alarm-Rushed Route Counter` is the selected route's primary opening counter bought while the pending `Boarding Alert` after that purchase is at least `4`, counting both Alert already pending and any same-purchase `Dockside Credit` Alert.
- `Alarm-Rushed Route Counter` applies only if that purchase is not already top-decked by `Full Crew Discount`, `Full Crew Discount` coverage, or `Opening Counter Prep`; it grants only top-deck placement, `Counter Watch`, and the normal route counter secured marker.
- `Alarm-Rushed Route Counter` does not grant `Opening Counter Prep` discount, prep `Might`, `Prepared` ship gains, `Route Starter Pass-Off`, `Cache Drill` bounty marks, Alert refunds, `Full Crew Discount` coverage, doubled `Ambush Bounty`, or any other cache reward.
- `Alarm-Rushed Route Counter` never applies to non-primary counters, non-counters, `Battle Test`, Boarding 2+, or purchases whose post-purchase pending Alert is below `4`.
...
- Before Boarding 1, the selected route's primary opening counter uses that `Top deck` exception only when the purchase spends `Full Crew Discount`, uses Full Crew coverage, consumes `Opening Counter Prep`, or qualifies as an `Alarm-Rushed Route Counter`; without one of those setup gates, the bought primary goes to discard even though it still counts as a scouted counter and still secures the route shop slot.
...
- `Top deck` scouted counter purchases without `Opening Counter Prep` or a post-Boarding-1 `Prepared` trigger still go on top of the draw pile, but are not `Prepared` and gain no prep `Might`; the pre-Boarding-1 selected route primary is the exception and needs `Full Crew Discount`, Full Crew coverage, `Opening Counter Prep`, or `Alarm-Rushed Route Counter` to be `Top deck`.

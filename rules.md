...
- During regular runs before Boarding 1, if the player successfully bought the selected route's primary opening counter before claiming that route's Boarding 1 `Scouted Counter Cache`, the cache also grants `+1☠️ Opening Route Contract`. This bonus is normal round shop currency, is granted once with that matching cache, adds no `Boarding Alert`, is not doubled by island bonuses, is not refunded by `Cache Drill`, and expires on Shop `Continue` like other `☠️`.
- `Opening Route Contract` never applies in `Battle Test`, after Boarding 1, to Boarding 2+ caches, to an unbought or wrong route primary, or if the matching primary is bought only after the cache was already claimed.

...
- `Opening Route Contract` is normal round shop currency, is granted at most once through the matching `Cache Drill`, adds no `Boarding Alert`, is not doubled by island bonuses, is not refunded by `Cache Drill`, and expires on Shop `Continue` like other `☠️`.
- `Opening Route Contract` never applies in `Battle Test`, after Boarding 1, to Boarding 2+ caches, to an unbought or wrong route primary, to late purchases after the cache was claimed, to removed pirates, or when an `Opening Deckhand Counter` starter or any other counter claims the drill first.
...
- Once the route primary is successfully bought before Boarding 1, the run marks that route counter as secured and records that specific bought pirate for the active `Opening Route Contract`. Remaining pre-Boarding-1 route-focused shop refills suppress all three opening counter candidates (`Poisoner`, `Sawbones`, `Needler`) and fill with distinct affordable non-counter starter shop options from `Drummer`, `Herald`, `Trainer`, and `Survivalist` where possible, falling back to normal unique eligible non-opening pirates only if needed.

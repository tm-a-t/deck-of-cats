...
- `Opening Counter Prep` is consumed by the first successful pirate purchase in that Shop. If that first purchase is a `Top deck` scouted counter, the new pirate goes on top of the draw pile, gains `Counter Watch`, and gains permanent `+1 💪 Might`.
...
- If the first purchase with `Opening Counter Prep` is not an eligible `Top deck` scouted counter, the prep is spent with no benefit.
...
- If `Opening Counter Prep` and `Full Crew Discount` are both present, the discount still changes the price as normal, the prep adds no extra discount, and both shop flags are consumed by the first successful pirate purchase. Before Boarding 1, the prep is the `+1 💪 Might` route; after Boarding 1 has started, `Full Crew Discount` keeps its normal `Prepared` trigger.
...
- `Opening Counter Prep` is consumed only by a successful pirate purchase, never by `Quiet Docks`, and expires on `Continue` if unused.

...
- Starting resources, `☠️`, `Boarding Alert`, `Full Crew Discount`, `Opening Counter Prep`, `Counter Watch`, the opening route counter secured marker, bought-pirate marker, prize-qualified marker, `Opening Route Muster`, and Cache Drill bounty marks: `0`/empty.
...
- Setup-gated opening route-primary buys report early only if that same purchase spends `Full Crew Discount`, uses `Full Crew Discount` coverage, or consumes `Opening Counter Prep`; those qualifying buys go on top of the draw pile and gain `Counter Watch`. A cash-only or Dockside Credit-only route-primary buy still secures the route shop slot, but goes to discard, gains no `Counter Watch`, gains no prep `Might`, and sets no `Opening Route Prize`.
- Only a route-primary purchase that consumes `Opening Counter Prep` marks that specific bought pirate for `Opening Route Prize`; `Full Crew Discount` and Full Crew coverage alone create the normal watched top-deck counter but do not qualify for the prize.
- `Opening Route Prize` lasts until Boarding 1 resolves: if that exact prep-qualified bought primary pirate triggers `Counter Ambush` against the selected route's main enemy, survives the winning opening combat hand, and normal `Ambush Bounty` would already be granted, that Ambush Bounty pays `+2` of the mapped cache resource instead of `+1`.
- `Opening Route Prize` does not stack above `+2` with a Cache Drill bounty mark; either qualifier makes the same Ambush Bounty pay `+2` total.
- `Opening Route Prize` never applies in `Battle Test`, losses, reinforcement-hand wins, Boarding 2+, wrong-main encounters, missing `Counter Ambush`, defeated ambushers, removed bought pirates, unbought or wrong route primaries, route-primary buys that did not consume `Opening Counter Prep`, non-primary counters, or any case where normal `Ambush Bounty` would not be granted. The prize marker clears when Boarding 1 resolves or when the marked pirate leaves the crew.
...
- Before Boarding 1, the selected route's primary opening counter uses that `Top deck` exception only when the purchase spends `Full Crew Discount`, uses Full Crew coverage, or consumes `Opening Counter Prep`; without one of those setup gates, the bought primary goes to discard even though it still counts as a scouted counter and still secures the route shop slot.
...
- If that `Opening Counter Prep` purchase is the selected route's primary opening counter before Boarding 1, that same bought pirate is also marked for `Opening Route Prize`.
...
- `Full Crew Discount` coverage is not spendable currency, does not use `Dockside Credit`, does not mark `shopCreditUsed`, and adds no `Boarding Alert`; the purchase spends the player's current `☠️`, consumes `Full Crew Discount`, goes on top of the draw pile, and gains `Counter Watch`, but the coverage and pre-Boarding-1 discount do not make it `Prepared`, do not grant prep `Might`, and do not qualify the route primary for `Opening Route Prize`.
...
- If that surviving ambusher has either a matching active Cache Drill bounty mark from the immediately preceding cache or an active Boarding 1 `Opening Route Prize`, `Ambush Bounty` pays `+2` of that resource instead of `+1`; having both still pays only `+2`. These marks are then gone with the resolved boarding.
- `Ambush Bounty` uses the cache resource map: `Shellback` → `🪵`, `Powder Bomber` → `🪨`, `Deck Sniper` → `🪙`, `Netter` → `🪵`, `Flint Duelist` → `🪵`.
- `Ambush Bounty` is granted once per boarding, never on losses, never in `Battle Test`, never if the ambusher was defeated, and never if the win comes from a reinforcement hand; those exclusions also prevent the doubled Cache Drill bounty and `Opening Route Prize`.

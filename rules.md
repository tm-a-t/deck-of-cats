...
- If that `Opening Counter Prep` purchase is the selected route's primary opening counter before Boarding 1, it still only gets the normal prep purchase effects; it must later claim the matching `Cache Drill` to double `Ambush Bounty`.

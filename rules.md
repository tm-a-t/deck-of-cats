...
- Setup-gated opening route-primary buys report early only if that same purchase spends `Full Crew Discount`, uses `Full Crew Discount` coverage, consumes `Opening Counter Prep`, or qualifies as a `Dockside Rush Route Counter`; those qualifying buys go on top of the draw pile and gain `Counter Watch`. A route-primary buy that meets none of those setup gates still secures the route shop slot, but goes to discard, gains no `Counter Watch`, and gains no prep `Might`.
...
- Before Boarding 1, the selected route's primary opening counter uses that `Top deck` exception only when the purchase spends `Full Crew Discount`, uses Full Crew coverage, consumes `Opening Counter Prep`, or qualifies as a `Dockside Rush Route Counter`; without one of those setup gates, the bought primary goes to discard even though it still counts as a scouted counter and still secures the route shop slot.

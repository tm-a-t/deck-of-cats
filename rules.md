...
- During regular runs before Boarding 1, buying the selected route's primary opening counter marks that specific bought pirate for `Opening Route Prize`.
- `Opening Route Prize` lasts until Boarding 1 resolves: if that exact bought primary pirate triggers `Counter Ambush` against the selected route's main enemy, survives the winning opening combat hand, and normal `Ambush Bounty` would already be granted, that Ambush Bounty pays `+2` of the mapped cache resource instead of `+1`.
...
- Once the route primary is successfully bought before Boarding 1, the run marks that route counter as secured and records that specific bought pirate for `Opening Route Prize`. Remaining pre-Boarding-1 route-focused shop refills suppress all three opening counter candidates (`Poisoner`, `Sawbones`, `Needler`) and fill with distinct affordable non-counter starter shop options from `Drummer`, `Herald`, `Trainer`, and `Survivalist` where possible, falling back to normal unique eligible non-opening pirates only if needed.

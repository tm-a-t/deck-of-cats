...
- Starting resources, `☠️`, `Boarding Alert`, `Full Crew Discount`, `Opening Counter Prep`, `Counter Watch`, the opening route cache-claimed marker, opening route counter secured marker, bought-pirate marker, `Opening Deckhand Scout Pay`, `Opening Route Muster`, and Cache Drill bounty marks: `0`/empty.
...
- `Opening Deckhand Scout Pay` happens once per regular run: on round `1`, on the layer-0 selected opening island, before Boarding 1, if the selected route's matching `Opening Deckhand Counter` starter is sent in any slot and is still in the crew after its own island action and any island removal, gain `+1☠️`.
...
- This any-sent-slot forgiveness affects only `Opening Deckhand Scout Pay`; cache opening, `Cache Drill`, `Route Starter Cache Prep`, cache Alert refunds, Cache Drill early report, `Route Starter Pass-Off`, `Secured Route Cache Pass-Off`, and doubled `Ambush Bounty` still require the starter to be the first cache opener where those rules say so.
- `Opening Deckhand Scout Pay` never applies in `Battle Test`, outside the layer-0 opening route island, after Boarding 1, after round `1`, to nonmatching starters, to matching starters that were not sent, to zero-send openings, or to starters removed by the island before the pay check.

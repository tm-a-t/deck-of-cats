...
- Starting resources, `☠️`, `Boarding Alert`, `Full Crew Discount`, `Opening Counter Prep`, `Counter Watch`, the opening route cache-claimed marker, opening route counter secured marker, bought-pirate marker, `Opening Route Muster`, and Cache Drill bounty marks: `0`/empty.
...
- During regular-run Boarding 1 only, when the selected layer-0 route cache is claimed by the first sent pirate and that route's primary opening counter has not yet been secured, the immediately following Shop gains normal `Opening Counter Prep`.
- Cache-granted `Opening Counter Prep` follows the normal purchase rules: the first eligible top-deck scouted-counter buy gets `-1☠️`, `+1 💪 Might`, top-deck placement, and `Counter Watch`; an eligible route side offer can spend it as `Opening Side Prep`; non-eligible non-counters and `Quiet Docks` do not consume it; and unused prep expires on Shop `Continue`.
- Cache-granted `Opening Counter Prep` is independent of `Cache Drill`: non-counter openers, wrong-starter openers, bought shop-counter openers, and removed openers can arm the prep by claiming the selected opening route cache, but they receive no `Cache Drill` rewards unless they separately satisfy the normal surviving counter requirements.
- Matching `Opening Deckhand Counter` starter openers keep the existing premium `Cache Drill` rewards: `+1 💪 Might`, Alert refund, early report, Cache Drill bounty mark, and eligible pass-off. They share the same single cache-granted `Opening Counter Prep` flag and never stack a second prep.
- Cache-granted `Opening Counter Prep` does not apply in `Battle Test`, Boarding 2+, non-opening caches, non-cache islands, zero-send skipped caches, unselected route caches, or routes whose primary opening counter is already secured.
- If the first opener is not a surviving eligible counter, the cache is claimed without `Cache Drill`; the stored `Boarding Alert` remains pending and later sent pirates cannot drill that cache, though an eligible Boarding 1 selected route cache can still grant `Opening Counter Prep` as above.
...
- During regular runs before Boarding 1, buying the selected route's primary opening counter marks that route counter as secured only when that same purchase goes on top of the draw pile and gains `Counter Watch` by spending `Full Crew Discount`, using `Full Crew Discount` coverage, consuming `Opening Counter Prep`, or qualifying as a `Cache-Claimed Alarm Rush Route Counter`.
- A route-primary buy that meets none of those setup gates still adds that pirate to discard, but does not mark the route secured, does not suppress the route-primary shop guarantee, does not block later cache-granted `Opening Counter Prep`, gains no `Counter Watch`, and gains no prep `Might`.
- Before Boarding 1 in regular runs, a `Cache-Claimed Alarm Rush Route Counter` is the selected route's primary opening counter bought in the same Shop after the selected route cache was claimed this round by sending at least one pirate, while the pending `Boarding Alert` after that purchase is at least `4`.
- Existing pending `Boarding Alert` alone never creates a `Cache-Claimed Alarm Rush Route Counter`; an unclaimed cache, skipped zero-send cache, non-route cache, or post-purchase pending Alert below `4` keeps an affordable cash buy in discard unless `Full Crew Discount`, `Full Crew Discount` coverage, or `Opening Counter Prep` applies.
- `Dockside Credit` can still help a claimed-cache route primary qualify by adding same-purchase pending Alert up to the `4` Alert threshold, but credit is not required when the claimed-cache cash purchase already has enough pending Alert.
- `Cache-Claimed Alarm Rush Route Counter` applies only if that purchase is not already top-decked by `Full Crew Discount`, `Full Crew Discount` coverage, or `Opening Counter Prep`; it grants only top-deck placement, `Counter Watch`, and the normal route counter secured marker.
- `Cache-Claimed Alarm Rush Route Counter` does not grant `Opening Counter Prep` discount, prep `Might`, `Prepared` ship gains, `Route Starter Pass-Off`, `Cache Drill` bounty marks, Alert refunds, `Full Crew Discount` coverage, doubled `Ambush Bounty`, or any other cache reward.
- `Cache-Claimed Alarm Rush Route Counter` never applies to unclaimed caches, zero-send skipped caches, non-primary counters, non-counters, `Battle Test`, Boarding 2+, or purchases whose post-purchase pending Alert is below `4`.
- If cache-granted `Opening Counter Prep` came from a matching starter-opened `Cache Drill` route cache and is spent on the selected route's primary opening counter in that immediately following Shop, `Route Starter Pass-Off` transfers the matching active Cache Drill bounty mark from the starter opener to the bought primary counter. The starter keeps its `Cache Drill` `+1 💪 Might`, Alert refund, early-report marker, and any `Counter Watch`; only the doubled-bounty mark moves.
...
- During regular-run Boarding 1 only, if the selected route's `Opening Deckhand Counter` starter successfully opens and drills the matching route cache after that route's primary opening counter has already been secured by a qualifying top-deck `Counter Watch` purchase, `Secured Route Cache Pass-Off` transfers the active Cache Drill bounty mark from the starter to that recorded bought primary counter if it is still owned.
...
- `Secured Route Cache Pass-Off` does not happen for `Battle Test`, Boarding 2+, wrong-main routes, failed Cache Drill, non-starter openers, missing or removed recorded bought primaries, discard-only route-primary buys, unsecure route-primary purchases, or bought primaries that are no longer owned.
...
- Until that route's primary opening counter is secured by a qualifying top-deck `Counter Watch` purchase, the visible 4-slot shop contains exactly 1 of `Poisoner`/`Sawbones`/`Needler`, and it must be the route primary. It also contains exactly 1 route side offer when eligible: `Forest Island`/`Shellback` → `Drummer`, `Rocky Island`/`Powder Bomber` → `Trainer`, and `Port Island`/`Deck Sniper` → `Survivalist`. The remaining slots are filled by distinct affordable non-counter starter shop options from `Drummer`, `Herald`, `Trainer`, and `Survivalist`, falling back to normal unique eligible generation only if needed.
- While that route's primary opening counter is not secured, the selected route side offer can spend active `Opening Counter Prep` as `Opening Side Prep`: after any `Full Crew Discount`, it gets the normal prep `-1☠️`, goes on top of the draw pile, consumes `Opening Counter Prep`, and gains an immediate permanent support buff. `Forest Island`/`Shellback` side-prepped `Drummer` gains `+1 ⚡ Tempo`; `Rocky Island`/`Powder Bomber` side-prepped `Trainer` gains `+1 💪 Might`; `Port Island`/`Deck Sniper` side-prepped `Survivalist` gains `+1 💪 Might`.
- `Opening Side Prep` does not secure the route, gain `Counter Watch` or `Watch Ready`, get counter prep `Might` unless its listed support buff is `Might`, create `Prepared`, trigger `Route Starter Pass-Off`, qualify for `Cache-Claimed Alarm Rush Route Counter`, create or move Cache Drill bounty marks, show counter payoff text, or receive counter ambush benefits.
- Opening route side offers without active eligible `Opening Side Prep` are ordinary non-counter purchases: they discard normally and do not consume `Opening Counter Prep`, go on top of the draw pile, gain `Counter Watch`, secure the route, or receive counter payoff perks.
...
- Before Boarding 1, the selected route's primary opening counter uses that `Top deck` exception only when the purchase spends `Full Crew Discount`, uses Full Crew coverage, consumes `Opening Counter Prep`, or qualifies as a `Cache-Claimed Alarm Rush Route Counter`; without one of those setup gates, the bought primary goes to discard even though it still counts as a scouted counter, and it does not secure the route shop slot.
...
- `Top deck` scouted counter purchases without `Opening Counter Prep` or a post-Boarding-1 `Prepared` trigger still go on top of the draw pile, but are not `Prepared` and gain no prep `Might`; the pre-Boarding-1 selected route primary is the exception and needs `Full Crew Discount`, Full Crew coverage, `Opening Counter Prep`, or `Cache-Claimed Alarm Rush Route Counter` to be `Top deck`.

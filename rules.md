...
- Opening route side offers are ordinary non-counter purchases: they do not secure the route, do not consume `Opening Counter Prep`, do not go on top of the draw pile, do not gain `Counter Watch`, do not get `Opening Prep` `Might`, do not trigger `Route Starter Pass-Off`, do not qualify for `Cache-Claimed Alarm Rush Route Counter`, and do not receive counter payoff perks.

...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === '--check-no-alarm-rush-route-counter' || a === '--check-alarm-rushed-route-counter' || a === '--check-dockside-rush-route-counter') {
      out.checkNoAlarmRushRouteCounter = true;
      continue;
    }
...
function shopPurchaseQuote(api, G, type) {
  ...
  const openingRoutePrimaryCommitment = !!(openingRoutePrimary
    ...
    && G.openingRoutePrimaryCommitmentMainKey === openingRouteMainKey);
...
function runOpeningRouteVictoryCacheChecks(runtime) {
  ...
  const setupBoarding = (route, opts = {}) => {
    ...
    if (routeEmoji && !opts.skipPlanCheck) {
      const choicePlan = scene.openingRouteChoicePlanLines(firstIsland).join(' ');
      const sendingPlan = scene.openingRoutePlanLines({ context: 'sending' }).join(' ');
      const expectedPreview = `clean +1${routeEmoji}/Alert +3${routeEmoji}`;
      assertOpeningRouteVictoryCacheCheck(choicePlan.includes(`B1 win ${expectedPreview}`), `${route.label} choice plan missed risk-scaled cache preview: ${choicePlan}`);
      assertOpeningRouteVictoryCacheCheck(!choicePlan.includes(`B1 win +2${routeEmoji}`), `${route.label} choice plan still hardcodes +2 cache win: ${choicePlan}`);
      assertOpeningRouteVictoryCacheCheck(sendingPlan.includes(`B1 cache ${expectedPreview}`), `${route.label} sending plan missed risk-scaled cache preview: ${sendingPlan}`);
      assertOpeningRouteVictoryCacheCheck(!sendingPlan.includes(`B1 cache +2${routeEmoji}`), `${route.label} sending plan still hardcodes +2 cache win: ${sendingPlan}`);
    }
    ...
    G.phase = 'map';
    if (opts.boardingAlertSnapshot != null) {
      G.boardingAlert = Math.max(0, Math.floor(Number(opts.boardingAlertSnapshot) || 0));
    }
    ...
    const activeAlert = G.enemyShip
      ? {
        amount: Math.max(0, Math.floor(Number(G.enemyShip.boardingAlert) || 0)),
        guardCount: Math.max(0, Math.floor(Number(G.enemyShip.boardingAlertGuards) || 0)),
      }
      : { amount: 0, guardCount: 0 };
    ...
    G.combat = {
      ...
      playerFighters: [],
      enemyFighters: [],
      boardingAlert: activeAlert.amount,
      boardingAlertGuards: activeAlert.guardCount,
      returnedPirateIds: [],
      reinforcementCount: Math.max(0, Math.floor(Number(opts.reinforcementCount) || 0)),
    };
  ...
  routes.forEach((route, index) => {
    ...
    scene.finishBoardingCombat('win');
    const expectedCount = Math.max(0, Math.floor(Number(G.enemyShip && G.enemyShip.boardingAlert) || 0)) > 0 ? 3 : 1;
    ...
    assertOpeningRouteVictoryCacheCheck(combat.openingRouteVictoryCache.count === expectedCount, `${route.label} wrong victory count ${JSON.stringify(combat.openingRouteVictoryCache)}`);
    assertOpeningRouteVictoryCacheCheck(resourceCount(G, route.resource) === before + expectedCount, `${route.label} paid ${resourceCount(G, route.resource) - before} instead of +${expectedCount} ${route.resource}`);
    ...
    assertOpeningRouteVictoryCacheCheck(resourceCount(G, route.resource) === before + expectedCount, `${route.label} duplicated reward after direct grant`);
    results.push({ name: `${route.label} claimed route cache pays exactly +${expectedCount} ${route.resource} on Boarding 1 win`, ok: true });
  });

  {
    const route = routes[0];
    const { G, opener, combat } = setupBoarding(route, { seed: 0x71c0a911, boardingAlertSnapshot: 1, reinforcementCount: 1 });
    combat.boardingAlertGuards = 0;
    combat.counterAmbush = {
      ...
      removedAlertGuardCount: 1,
      removedAlertGuards: [{ key: 'cabinBoy' }],
    };
    const before = resourceCount(G, route.resource);
    scene.finishBoardingCombat('win');
    assertOpeningRouteVictoryCacheCheck(combat.openingRouteVictoryCache && combat.openingRouteVictoryCache.count === 3, `forced Alert win did not pay +3: ${JSON.stringify(combat.openingRouteVictoryCache)}`);
    assertOpeningRouteVictoryCacheCheck(resourceCount(G, route.resource) === before + 3, `forced Alert win paid ${resourceCount(G, route.resource) - before} instead of +3`);
    results.push({ name: 'forced Alert Boarding 1 win pays +3 even after Alert guards are removed', ok: true });
  }

  {
    const route = routes[0];
    const { G, opener, combat } = setupBoarding(route, { seed: 0x71c0b001, boardingAlertSnapshot: 1 });
    ...
    assertOpeningRouteVictoryCacheCheck(combat.openingRouteVictoryCache && combat.openingRouteVictoryCache.count === 3, 'stack check missed risk-scaled Opening Route Victory Cache');
  ...
  [
    ...
    const resource = negative.resource || routes[1].resource;
    const before = resourceCount(setup.G, resource);
    ...
    assertOpeningRouteVictoryCacheCheck(resourceCount(setup.G, resource) === before, `${negative.name} paid victory cache`);
...
function runNoAlarmRushRouteCounterChecks(runtime) {
  const api = runtime.api;
  const scene = makeSimScene(api);
  const results = [];
  const routeCases = [
  ...
  const assertSceneAndPolicyMatch = (G, type, label) => {
    ...
    assertNoAlarmRushRouteCounterCheck(
      !!quote.alarmRushedRouteCounter === !!directQuote.alarmRushedRouteCounter
        ...
        && !!quote.openingRoutePrimaryCommitment === !!directQuote.openingRoutePrimaryCommitment
        && (quote.routeCounterCover || 0) === (directQuote.routeCounterCover || 0)
        && (quote.alert || 0) === (directQuote.alert || 0),
      `${label} scene/direct quote mismatch: ${JSON.stringify({ quote, directQuote })}`
    );
    assertNoAlarmRushRouteCounterCheck(!quote.alarmRushedRouteCounter, `${label} exposed retired alarm-rush flag: ${JSON.stringify(quote)}`);
    return quote;
  };

  {
    ...
    assertNoAlarmRushRouteCounterCheck(opener, 'Port claimed-cache setup did not find a non-counter opener');
    G.sent = [openerIndex];
    const claim = scene.claimScoutedCounterCache(opener, { silent: true });
    assertNoAlarmRushRouteCounterCheck(claim && claim.cacheGrant && !claim.drill, `Port non-counter opener claim mismatch: ${JSON.stringify(claim)}`);
    assertNoAlarmRushRouteCounterCheck(routeCache && routeCache.scoutedCache && routeCache.scoutedCache.claimed, 'Port cache node was not marked claimed');
    assertNoAlarmRushRouteCounterCheck(G.island && G.island.scoutedCacheDrill && G.island.scoutedCacheDrill.cacheClaimed, 'Port island drill state was not marked claimed');
    assertNoAlarmRushRouteCounterCheck(!claim.openingCounterPrep && G.openingCounterPlan === false, 'non-counter Port cache claim granted Opening Counter Prep');
    const wagePreview = finishIslandForShop(G, [openerIndex]);
    assertNoAlarmRushRouteCounterCheck(G.enthusiasm >= api.TYPES[route.primary].cost && G.boardingAlert === 5, `Port claimed-cache state mismatch: ${JSON.stringify({ enthusiasm: G.enthusiasm, alert: G.boardingAlert, wagePreview })}`);
    const quote = assertSceneAndPolicyMatch(G, route.primary, 'Port claimed-cache high Alert primary');
    assertNoAlarmRushRouteCounterCheck(quote.canBuy && quote.counter && quote.openingRoutePrimary && !quote.credit, `Port claimed-cache quote not cash-buyable primary: ${JSON.stringify(quote)}`);
    assertNoAlarmRushRouteCounterCheck(quote.topDeck && quote.openingRoutePrimaryCommitment && !quote.openingCounterPrepMight && quote.routeCounterCover === 0, `Port claimed-cache high Alert primary missed commitment or gained setup: ${JSON.stringify(quote)}`);
    assertNoAlarmRushRouteCounterCheck(!quote.preparedCounter && quote.fullCrewCoverage === 0, `Port claimed-cache high Alert gained excluded setup perks: ${JSON.stringify(quote)}`);
    const plan = scene.shopPlanText();
    assertNoAlarmRushRouteCounterCheck(!plan.includes('Alarm rush') && plan.includes('route commitment') && plan.includes('top deck'), `Port claimed-cache plan missed commitment top-deck or labels retired alarm: ${plan}`);
    const bought = scene.buyPirate(G.shop.indexOf(route.primary), { deferRender: true, silent: true, ignoreAnimating: true, skipPanelRefresh: true });
    assertNoAlarmRushRouteCounterCheck(bought && bought.type === route.primary, 'Port claimed-cache cash buy failed');
    assertNoAlarmRushRouteCounterCheck(G.deck.includes(bought) && !G.discard.includes(bought), 'Port claimed-cache commitment buy did not top-deck');
    assertNoAlarmRushRouteCounterCheck((G.counterWatchIds || []).includes(bought.id), 'Port claimed-cache commitment buy missed Counter Watch');
    assertNoAlarmRushRouteCounterCheck((bought.might || 0) === 0 && (bought.tempo || 0) === 0 && !bought.weaponKey, `Port claimed-cache high Alert buy gained prep stats: ${JSON.stringify(bought)}`);
    assertNoAlarmRushRouteCounterCheck((G.cacheDrillBountyMarks || []).length === 0, 'Port claimed-cache cash buy created Cache Drill marks');
    assertNoAlarmRushRouteCounterCheck(G.boardingAlert === 5, `Port claimed-cache cash buy changed Alert through retired cover: ${G.boardingAlert}`);
    assertNoAlarmRushRouteCounterCheck(G.openingRouteCounterBoughtMainKey === route.mainKey && G.openingRouteCounterBoughtPirateId === bought.id, 'Port claimed-cache commitment buy did not secure route counter');
    results.push({ name: 'claimed Port cache high Alert first primary uses route commitment without Alert-rush Cover or setup stats', ok: true, quote, plan, wagePreview });
  }

  {
    ...
    assertNoAlarmRushRouteCounterCheck(quote.canBuy && quote.counter && quote.openingRoutePrimary, `Port quote not buyable primary counter: ${JSON.stringify(quote)}`);
    assertNoAlarmRushRouteCounterCheck(!quote.topDeck && !quote.credit && quote.alert === 0 && quote.routeCounterCover === 0, `Port cash high Alert became setup-gated: ${JSON.stringify(quote)}`);
    assertNoAlarmRushRouteCounterCheck(!quote.openingCounterPrepMight && !quote.preparedCounter && quote.fullCrewCoverage === 0, `Port cash high Alert gained setup perks: ${JSON.stringify(quote)}`);
    const plan = scene.shopPlanText();
    assertNoAlarmRushRouteCounterCheck(!plan.includes('Alarm rush') && !plan.includes('top deck, Watch'), `Port shop plan mislabeled claimed cash high Alert setup: ${plan}`);
    const bought = scene.buyPirate(G.shop.indexOf(route.primary), { deferRender: true, silent: true, ignoreAnimating: true, skipPanelRefresh: true });
    assertNoAlarmRushRouteCounterCheck(bought && bought.type === route.primary, 'Port high-Alert cash buy failed');
    assertNoAlarmRushRouteCounterCheck(G.discard.includes(bought) && !G.deck.includes(bought), 'Port high-Alert cash buy did not discard');
    assertNoAlarmRushRouteCounterCheck(!(G.counterWatchIds || []).includes(bought.id), 'Port high-Alert cash buy gained Counter Watch');
    assertNoAlarmRushRouteCounterCheck((bought.might || 0) === 0 && (bought.tempo || 0) === 0 && !bought.weaponKey, `Port high-Alert cash buy gained prep/Prepared upgrades: ${JSON.stringify(bought)}`);
    assertNoAlarmRushRouteCounterCheck((G.cacheDrillBountyMarks || []).length === 0, 'Port high-Alert cash buy created Cache Drill marks');
    assertNoAlarmRushRouteCounterCheck(G.boardingAlert === 4 && !G.openingRouteCounterBoughtMainKey && G.openingRouteCounterBoughtPirateId == null, `Port high-Alert cash buy changed Alert or secured route: ${G.boardingAlert}/${G.openingRouteCounterBoughtMainKey}`);
    results.push({ name: 'claimed-cache cash primary at 4 pending Alert stays unsecured and discards without Watch', ok: true, quote, plan });
  }

  {
    ...
    assertNoAlarmRushRouteCounterCheck(wagePreview.wages === 4 && wagePreview.alert === 3, `Port zero-send wages mismatch: ${JSON.stringify(wagePreview)}`);
    assertNoAlarmRushRouteCounterCheck(G.enthusiasm >= api.TYPES[route.primary].cost && G.boardingAlert === 3, `Port zero-send state mismatch: ${JSON.stringify({ enthusiasm: G.enthusiasm, alert: G.boardingAlert })}`);
    const quote = assertSceneAndPolicyMatch(G, route.primary, 'Port cash below threshold');
    assertNoAlarmRushRouteCounterCheck(quote.canBuy && !quote.alarmRushedRouteCounter && quote.topDeck && quote.openingRoutePrimaryCommitment, `below-threshold cash primary missed commitment: ${JSON.stringify(quote)}`);
    const bought = scene.buyPirate(G.shop.indexOf(route.primary), { deferRender: true, silent: true, ignoreAnimating: true, skipPanelRefresh: true });
    assertNoAlarmRushRouteCounterCheck(G.deck.includes(bought) && !G.discard.includes(bought), 'below-threshold commitment primary did not top-deck');
    assertNoAlarmRushRouteCounterCheck((G.counterWatchIds || []).includes(bought.id), 'below-threshold commitment primary missed Counter Watch');
    assertNoAlarmRushRouteCounterCheck(G.openingRouteCounterBoughtMainKey === route.mainKey && G.openingRouteCounterBoughtPirateId === bought.id, 'below-threshold commitment primary did not secure route counter');
    results.push({ name: 'Port zero-send first primary uses route commitment without setup Might or Cover', ok: true, quote, wagePreview });
  }

  {
    ...
    assertNoAlarmRushRouteCounterCheck(quote.canBuy && !quote.credit && !quote.alarmRushedRouteCounter && !quote.topDeck, `claimed-cache cash below threshold top-decked: ${JSON.stringify(quote)}`);
    const bought = scene.buyPirate(G.shop.indexOf(route.primary), { deferRender: true, silent: true, ignoreAnimating: true, skipPanelRefresh: true });
    assertNoAlarmRushRouteCounterCheck(G.discard.includes(bought) && !G.deck.includes(bought), 'claimed-cache cash below threshold did not discard');
    assertNoAlarmRushRouteCounterCheck(!(G.counterWatchIds || []).includes(bought.id), 'claimed-cache cash below threshold gained Counter Watch');
    assertNoAlarmRushRouteCounterCheck(!G.openingRouteCounterBoughtMainKey && G.openingRouteCounterBoughtPirateId == null, 'claimed-cache cash below threshold secured route counter');
...

  {
    ...
    const quote = assertSceneAndPolicyMatch(G, route.primary, 'Port claimed-cache Dockside Credit');
    assertNoAlarmRushRouteCounterCheck(quote.canBuy && quote.credit && quote.alert === 2, `credit route primary did not expose +2 Alert: ${JSON.stringify(quote)}`);
    assertNoAlarmRushRouteCounterCheck(!quote.topDeck && quote.routeCounterCover === 0, `Dockside Credit route primary top-decked or covered without setup: ${JSON.stringify(quote)}`);
    const plan = scene.shopPlanText();
    assertNoAlarmRushRouteCounterCheck(plan.includes('credit +2 Alert') && !plan.includes('Alarm rush') && !plan.includes('top deck'), `credit plan mislabeled retired setup: ${plan}`);
    const bought = scene.buyPirate(G.shop.indexOf(route.primary), { deferRender: true, silent: true, ignoreAnimating: true, skipPanelRefresh: true });
    assertNoAlarmRushRouteCounterCheck(G.boardingAlert === 4 && G.shopCreditUsed === true, `credit Alert/shopCredit mismatch without Cover: ${G.boardingAlert}/${G.shopCreditUsed}`);
    assertNoAlarmRushRouteCounterCheck(G.discard.includes(bought) && !G.deck.includes(bought), 'credit route primary did not discard');
    assertNoAlarmRushRouteCounterCheck(!(G.counterWatchIds || []).includes(bought.id), 'credit route primary gained Counter Watch');
    assertNoAlarmRushRouteCounterCheck((bought.might || 0) === 0 && !bought.weaponKey && (bought.tempo || 0) === 0, `credit route primary gained excluded upgrades: ${JSON.stringify(bought)}`);
    assertNoAlarmRushRouteCounterCheck(!G.openingRouteCounterBoughtMainKey && G.openingRouteCounterBoughtPirateId == null, 'credit route primary secured route counter');
    results.push({ name: 'Dockside Credit can buy a claimed-cache primary but never top-decks or secures it by Alert alone', ok: true, quote, plan });
  }

  {
    ...
    assertNoAlarmRushRouteCounterCheck(quote.canBuy && quote.credit && quote.alert === 2 && !quote.alarmRushedRouteCounter && !quote.topDeck, `unclaimed credit route primary top-decked: ${JSON.stringify(quote)}`);
    const bought = scene.buyPirate(G.shop.indexOf(route.primary), { deferRender: true, silent: true, ignoreAnimating: true, skipPanelRefresh: true });
    assertNoAlarmRushRouteCounterCheck(G.boardingAlert === 4 && G.discard.includes(bought) && !G.deck.includes(bought), 'unclaimed credit primary did not discard with +2 Alert');
    assertNoAlarmRushRouteCounterCheck(!(G.counterWatchIds || []).includes(bought.id), 'unclaimed credit primary gained Counter Watch');
    assertNoAlarmRushRouteCounterCheck(!G.openingRouteCounterBoughtMainKey && G.openingRouteCounterBoughtPirateId == null, 'unclaimed credit primary secured route counter');
    results.push({ name: 'unclaimed route caches also discard credit route primaries at 4 projected Alert', ok: true, quote });
  }

  {
    ...
    assertNoAlarmRushRouteCounterCheck(quote.canBuy && quote.credit && quote.alert === 2 && !quote.alarmRushedRouteCounter && !quote.topDeck, `credit below threshold top-decked: ${JSON.stringify(quote)}`);
    const bought = scene.buyPirate(G.shop.indexOf(route.primary), { deferRender: true, silent: true, ignoreAnimating: true, skipPanelRefresh: true });
    assertNoAlarmRushRouteCounterCheck(G.boardingAlert === 3 && G.discard.includes(bought) && !G.deck.includes(bought), 'credit below threshold did not discard with +2 Alert');
    assertNoAlarmRushRouteCounterCheck(!(G.counterWatchIds || []).includes(bought.id), 'credit below threshold gained Counter Watch');
    assertNoAlarmRushRouteCounterCheck(!G.openingRouteCounterBoughtMainKey && G.openingRouteCounterBoughtPirateId == null, 'credit below threshold secured route counter');
...

  {
    ...
    assertNoAlarmRushRouteCounterCheck(quote.canBuy && quote.topDeck && quote.discount === 1, `Full Crew setup no longer top-decks: ${JSON.stringify(quote)}`);
    const bought = scene.buyPirate(G.shop.indexOf(route.primary), { deferRender: true, silent: true, ignoreAnimating: true, skipPanelRefresh: true });
    assertNoAlarmRushRouteCounterCheck(G.deck.includes(bought) && (G.counterWatchIds || []).includes(bought.id) && G.openingRouteCounterBoughtMainKey === route.mainKey, 'Full Crew setup did not secure watched top-deck counter');
    results.push({ name: 'Full Crew setup still top-decks, Watches, and secures the route primary', ok: true, quote });
  }

  {
    ...
    assertNoAlarmRushRouteCounterCheck(quote.canBuy && quote.topDeck && quote.openingCounterPrepMight, `Opening Prep setup no longer top-decks: ${JSON.stringify(quote)}`);
    const bought = scene.buyPirate(G.shop.indexOf(route.primary), { deferRender: true, silent: true, ignoreAnimating: true, skipPanelRefresh: true });
    assertNoAlarmRushRouteCounterCheck((bought.might || 0) === 1 && G.deck.includes(bought) && (G.counterWatchIds || []).includes(bought.id) && G.openingRouteCounterBoughtMainKey === route.mainKey, 'Opening Prep no longer granted prep Might/top-deck/route security');
    results.push({ name: 'Opening Counter Prep remains the route-primary prep Might and security path', ok: true, quote });
  }

  {
    const route = { label: 'Port/Deck Sniper', mainKey: 'deckSniper', primary: 'needler', sideOffer: 'survivalist' };
    const sideCost = api.TYPES[route.sideOffer].cost;
    const primaryCost = api.TYPES[route.primary].cost;
    const G = setupRouteShop(route, {
      boardingAlert: 4,
      openingCounterPlan: true,
      cacheClaimed: true,
      enthusiasm: sideCost + primaryCost,
    });
    G.shop = api.normalizeOpeningRouteShop([route.primary, route.sideOffer, 'herald', 'trainer'], G.round, {
      map: G.map,
      mode: G.mode,
      boardingCount: G.boardingCount,
    });
    const sideQuote = assertSceneAndPolicyMatch(G, route.sideOffer, 'Port side prep before primary');
    assertNoAlarmRushRouteCounterCheck(sideQuote.canBuy && sideQuote.openingSidePrep && sideQuote.topDeck && !sideQuote.counter, `Opening Side Prep unavailable: ${JSON.stringify(sideQuote)}`);
    const sidekick = scene.buyPirate(G.shop.indexOf(route.sideOffer), { deferRender: true, silent: true, ignoreAnimating: true, skipPanelRefresh: true });
    assertNoAlarmRushRouteCounterCheck(sidekick && sidekick.type === route.sideOffer && !G.openingCounterPlan, 'Opening Side Prep buy failed or did not consume prep');
    assertNoAlarmRushRouteCounterCheck(!G.openingRouteCounterBoughtMainKey && !(G.counterWatchIds || []).includes(sidekick.id), 'Opening Side Prep secured route or gave Watch');
    const primaryQuote = assertSceneAndPolicyMatch(G, route.primary, 'Port primary after side prep');
    assertNoAlarmRushRouteCounterCheck(primaryQuote.canBuy && !primaryQuote.topDeck && !primaryQuote.openingCounterPrepMight && primaryQuote.routeCounterCover === 0, `primary after side prep gained retired setup: ${JSON.stringify(primaryQuote)}`);
    const bought = scene.buyPirate(G.shop.indexOf(route.primary), { deferRender: true, silent: true, ignoreAnimating: true, skipPanelRefresh: true });
    assertNoAlarmRushRouteCounterCheck(bought && G.discard.includes(bought) && !G.deck.includes(bought), 'primary after side prep did not discard');
    assertNoAlarmRushRouteCounterCheck(!(G.counterWatchIds || []).includes(bought.id) && !G.openingRouteCounterBoughtMainKey, 'primary after side prep gained Watch or secured route');
    results.push({ name: 'spending Opening Side Prep cannot be followed by Alert-secured primary in the same shop', ok: true, sideQuote, primaryQuote });
  }

  {
    ...
    assertNoAlarmRushRouteCounterCheck(!quote.counter && !quote.topDeck, `non-counter received route setup: ${JSON.stringify(quote)}`);
    results.push({ name: 'non-counter buys never become route counters from Alert alone', ok: true, quote });
  }

  {
    ...
    assertNoAlarmRushRouteCounterCheck(!quote.openingRoutePrimary && !quote.alarmRushedRouteCounter, `Boarding 2+ received opening route setup: ${JSON.stringify(quote)}`);
    results.push({ name: 'Boarding 2+ route-primary buys cannot use opening Alert security setup', ok: true, quote });
  }

  {
    ...
    assertNoAlarmRushRouteCounterCheck(!quote.openingRoutePrimary && !quote.topDeck, `Battle Test received opening route setup: ${JSON.stringify(quote)}`);
    results.push({ name: 'Battle Test purchases cannot use opening Alert setup', ok: true, quote });
  }
...
function runRouteCounterCoverChecks(runtime) {
...

  {
    ...
    const quote = scene.shopPurchaseQuote(route.primary);
    assertRouteCounterCoverCheck(quote.canBuy && !quote.topDeck && !quote.alarmRushedRouteCounter && quote.routeCounterCover === 0, `retired cache-claimed Alert path still previewed Cover: ${JSON.stringify(quote)}`);
    const bought = scene.buyPirate(G.shop.indexOf(route.primary), { deferRender: true, silent: true, ignoreAnimating: true, skipPanelRefresh: true });
    assertRouteCounterCoverCheck(bought && bought.type === route.primary && G.discard.includes(bought) && !G.deck.includes(bought), 'retired cache-claimed Alert buy did not discard');
    assertRouteCounterCoverCheck(G.boardingAlert === 4 && !G.openingRouteCounterBoughtMainKey, `retired cache-claimed Alert buy changed Alert or secured route ${G.boardingAlert}/${G.openingRouteCounterBoughtMainKey}`);
    results.push({ name: 'claimed-cache high Alert route-primary buy no longer triggers Route Counter Cover', ok: true, quote });
  }
...
function runOpeningRouteCounterShopChecks(runtime) {
  ...
  const assertRouteFocusedShop = (shop, label, primary, opts = {}) => {
    ...
    assertOpeningRouteCounterShopCheck(
      shop[0] === primary,
      `${label} route primary ${primary} should be pinned to slot 0, got ${shop.join(',')}`
    );
    if (expectSideOffer) {
      assertOpeningRouteCounterShopCheck(
  ...
  routeCases.forEach(({ label, mainKey, primary }, routeIndex) => {
    ...
    G.boardingAlert = 2;
    scene.enterShoppingPhase();
    assertRouteFocusedShop(G.shop, `${label} commitment shop`, primary);
    assertOpeningRouteCounterShopCheck(G.openingRoutePrimaryCommitmentMainKey === mainKey, `${label} commitment did not arm for shop`);
    const quote = scene.shopPurchaseQuote(primary);
    const directQuote = shopPurchaseQuote(api, G, primary);
    assertOpeningRouteCounterShopCheck(
      quote.canBuy
        && quote.counter
        && quote.topDeck
        ...
        && !quote.openingCounterPrepMight
        && !quote.preparedCounter
        && quote.routeCounterCover === 0,
      `${label} cash commitment quote mismatch: ${JSON.stringify(quote)}`
    );
    assertOpeningRouteCounterShopCheck(
      directQuote.canBuy && directQuote.topDeck && directQuote.openingRoutePrimaryCommitment && directQuote.routeCounterCover === 0,
      `${label} direct commitment quote mismatch: ${JSON.stringify(directQuote)}`
    );
    const bought = scene.buyPirate(0, { deferRender: true, silent: true, ignoreAnimating: true, skipPanelRefresh: true });
    assertOpeningRouteCounterShopCheck(bought && bought.type === primary, `${label} commitment primary buy failed`);
    assertOpeningRouteCounterShopCheck(G.deck[G.deck.length - 1] === bought && !G.discard.includes(bought), `${label} commitment primary did not top-deck`);
    assertOpeningRouteCounterShopCheck((G.counterWatchIds || []).includes(bought.id), `${label} commitment primary missed Counter Watch`);
    assertOpeningRouteCounterShopCheck(G.openingRouteCounterBoughtMainKey === mainKey && G.openingRouteCounterBoughtPirateId === bought.id, `${label} commitment primary did not secure route`);
    assertOpeningRouteCounterShopCheck(!bought.weaponKey && (bought.might || 0) === 0 && (bought.tempo || 0) === 0, `${label} commitment primary gained setup stats`);
    assertOpeningRouteCounterShopCheck(G.boardingAlert === 2, `${label} commitment primary incorrectly triggered Route Counter Cover`);
    assertOpeningRouteCounterShopCheck(G.openingRoutePrimaryCommitmentMainKey == null, `${label} commitment did not clear after primary buy`);
    results.push({ name: `${label} first route-primary purchase commits without prep, top-decks, Watches, and secures without Cover`, ok: true, quote });
  });

  {
    ...
    G.fullCrewDiscount = 0;
    G.openingCounterPlan = false;
    G.enthusiasm = api.TYPES[route.primary].cost - 2;
    G.boardingAlert = 1;
    scene.enterShoppingPhase();
    const quote = scene.shopPurchaseQuote(route.primary);
    assertOpeningRouteCounterShopCheck(
      quote.canBuy
        ...
        && quote.routeCounterCover === 0
        && !quote.openingCounterPrepMight,
      `Dockside commitment quote mismatch: ${JSON.stringify(quote)}`
    );
    const bought = scene.buyPirate(0, { deferRender: true, silent: true, ignoreAnimating: true, skipPanelRefresh: true });
    assertOpeningRouteCounterShopCheck(bought && bought.type === route.primary, 'Dockside commitment buy failed');
    assertOpeningRouteCounterShopCheck(G.boardingAlert === 3 && G.shopCreditUsed === true, `Dockside commitment Alert mismatch: ${G.boardingAlert}/${G.shopCreditUsed}`);
    assertOpeningRouteCounterShopCheck(G.deck[G.deck.length - 1] === bought && (G.counterWatchIds || []).includes(bought.id), 'Dockside commitment did not top-deck and Watch');
    assertOpeningRouteCounterShopCheck(G.openingRouteCounterBoughtMainKey === route.mainKey && G.openingRouteCounterBoughtPirateId === bought.id, 'Dockside commitment did not secure route');
    assertOpeningRouteCounterShopCheck((bought.might || 0) === 0 && !bought.weaponKey && (bought.tempo || 0) === 0, `Dockside commitment gained setup stats: ${JSON.stringify(bought)}`);
    results.push({ name: 'Dockside Credit first route-primary commitment top-decks and secures while adding normal Alert without Cover', ok: true, quote });
  }

  {
    ...
    G.fullCrewDiscount = 0;
    G.openingCounterPlan = false;
    G.enthusiasm = api.TYPES[route.primary].cost + api.TYPES.herald.cost;
    G.boardingAlert = 2;
    scene.enterShoppingPhase();
    assertOpeningRouteCounterShopCheck(G.shop[0] === route.primary && G.shop.includes('herald'), `missed commitment setup shop mismatch: ${G.shop.join(',')}`);
    const nonPrimary = scene.buyPirate(G.shop.indexOf('herald'), { deferRender: true, silent: true, ignoreAnimating: true, skipPanelRefresh: true });
    assertOpeningRouteCounterShopCheck(nonPrimary && nonPrimary.type === 'herald', 'non-primary first buy failed');
    assertOpeningRouteCounterShopCheck(G.openingRoutePrimaryCommitmentMainKey == null, 'non-primary first buy did not clear commitment');
    assertOpeningRouteCounterShopCheck(!G.openingRouteCounterBoughtMainKey && G.openingRouteCounterBoughtPirateId == null, 'non-primary first buy secured route');
    assertRouteFocusedShop(G.shop, 'missed commitment refill', route.primary);
    const quote = scene.shopPurchaseQuote(route.primary);
    assertOpeningRouteCounterShopCheck(
      quote.canBuy && quote.counter && !quote.topDeck && !quote.openingRoutePrimaryCommitment && quote.routeCounterCover === 0,
      `missed commitment later primary quote mismatch: ${JSON.stringify(quote)}`
    );
    const bought = scene.buyPirate(0, { deferRender: true, silent: true, ignoreAnimating: true, skipPanelRefresh: true });
    assertOpeningRouteCounterShopCheck(bought && bought.type === route.primary, 'missed commitment later primary buy failed');
    assertOpeningRouteCounterShopCheck(G.discard.includes(bought) && !G.deck.includes(bought), 'missed commitment later primary did not discard');
    assertOpeningRouteCounterShopCheck(!(G.counterWatchIds || []).includes(bought.id), 'missed commitment later primary gained Counter Watch');
    assertOpeningRouteCounterShopCheck(!G.openingRouteCounterBoughtMainKey && G.openingRouteCounterBoughtPirateId == null, 'missed commitment later primary secured route');
    results.push({ name: 'buying a non-primary first misses the route-primary commitment for the rest of that Shop', ok: true, quote });
  }
...
function runOpeningCachePurseChecks(runtime) {
...

  {
    ...
    assertOpeningCachePurseCheck(G.openingCounterPlan === false && !cacheClaim.openingCounterPrep && !cacheClaim.drill.openingCounterPrep, 'same-round cache drill armed Opening Counter Prep');
  ...
  routes.forEach((route) => {
    ...
    assertOpeningCachePurseCheck(quote.canBuy && !quote.credit && quote.discount === 1 && quote.topDeck && !quote.openingCounterPrepMight, `${route.label} primary quote not Full Crew top-decked without prep/credit: ${JSON.stringify(quote)}`);
    const bought = scene.buyPirate(0, { deferRender: true, silent: true, ignoreAnimating: true, skipPanelRefresh: true });
    ...
    assertOpeningCachePurseCheck((bought.might || 0) === 0 && !bought.weaponKey && (bought.tempo || 0) === 0, `${route.label} Full Crew route primary gained prep/prepared stats: ${JSON.stringify(bought)}`);
    assertOpeningCachePurseCheck((G.cacheDrillBountyMarks || []).some(mark => mark && mark.pirateId === bought.id && mark.mainKey === route.mainKey), `${route.label} Full Crew route primary did not receive starter bounty mark: ${JSON.stringify({ bought, marks: G.cacheDrillBountyMarks })}`);
    assertOpeningCachePurseCheck((pirates[0].might || 0) === 1 && (G.cacheDrillMusterIds || []).includes(pirates[0].id), `${route.label} starter lost Cache Drill Might or early report after Full Crew pass-off`);
    results.push({ name: `${route.label} full-send cache purse funds a Full Crew route counter buy and passes the starter bounty mark`, ok: true });
  });

  {
    ...
    assertOpeningCachePurseCheck(!claim.openingCounterPrep && G.openingCounterPlan === false, 'nonmatching first opener armed Opening Counter Prep from cache');
    G.phase = 'shopping';
    G.busy = false;
    G.shopAnimating = false;
    G.shopCreditUsed = false;
    ...
    const quote = scene.shopPurchaseQuote(route.primary);
    assertOpeningCachePurseCheck(quote.canBuy && !quote.topDeck && !quote.openingCounterPrepMight && !quote.alarmRushedRouteCounter, `nonmatching cache opener exposed primary setup perks: ${JSON.stringify(quote)}`);
    const sideQuote = scene.shopPurchaseQuote(sideOffer);
    assertOpeningCachePurseCheck(sideQuote.canBuy && !sideQuote.openingSidePrep && !sideQuote.topDeck && !sideQuote.counter, `nonmatching cache opener exposed side Opening Prep: ${JSON.stringify(sideQuote)}`);
    const side = scene.buyPirate(1, { deferRender: true, silent: true, ignoreAnimating: true, skipPanelRefresh: true });
    assertOpeningCachePurseCheck(side && side.type === sideOffer && G.discard.includes(side) && !G.deck.includes(side), 'nonmatching cache side offer did not discard normally');
    assertOpeningCachePurseCheck((side.might || 0) + (side.tempo || 0) === 0 && !(G.counterWatchIds || []).includes(side.id), `nonmatching cache side offer gained setup state: ${JSON.stringify(side)}`);
    assertOpeningCachePurseCheck((G.cacheDrillBountyMarks || []).length === 0, 'nonmatching cache side buy created Cache Drill bounty marks');
    results.push({ name: 'nonmatching first opener receives the cache purse without Opening Prep or Cache Drill', ok: true, quote, sideQuote });
  }

  {
    ...
    assertOpeningCachePurseCheck(!prepAfterFirst && G.openingCounterPlan === false, 'second-slot starter changed the first opener no-prep flag');
...

  {
    ...
    assertOpeningCachePurseCheck(!claim.openingCounterPrep && G.openingCounterPlan === false, 'removed first opener armed Opening Prep from cache');
    results.push({ name: 'removed first openers still claim the visible cache purse but cannot drill or prep', ok: true });
  }
...
function runOpeningDeckhandCounterChecks(runtime) {
  ...
  const setupCache = (route, starterType, opts = {}) => {
    ...
    G.openingRouteCounterBoughtSetup = opts.boughtSetup != null ? !!opts.boughtSetup : !!opts.boughtMainKey;
    scene._sacrificedIds.clear();
    return { G, pirate };
  };
  ...
  const assertBoughtPrimaryDoubledBountyAfterPassOff = (G, route, starter, bought, label) => {
    G.res = { wood: 0, stone: 0, gold: 0 };
    const combat = setupPassOffCombat(G, route, starter, bought);
    combat.playerSetupRows = scene.combatDefaultPlayerSetupRows(combat);
    assertOpeningDeckhandCounterCheck(combat.playerSetupRows[0][0] === bought.id, `${label} bought primary did not default front-left after pass-off`);
    combat.playerFighters = scene.buildPlayerCombatFighters(combat.playerSetupRows, combat);
    const ambush = scene.applyCounterAmbush(combat, { silent: true });
    assertOpeningDeckhandCounterCheck(ambush && ambush.pirateId === bought.id, `${label} bought primary did not Counter Ambush from default setup`);
    combat.result = 'win';
    const bounty = scene.grantAmbushBounty(combat);
    assertOpeningDeckhandCounterCheck(bounty && bounty.pirateId === bought.id && bounty.count === 2 && bounty.drilled, `${label} bought primary did not earn doubled bounty: ${JSON.stringify(bounty)}`);
    assertOpeningDeckhandCounterCheck(G.res[route.bountyRes] === 2, `${label} doubled bounty resource wrong: ${JSON.stringify(G.res)}`);
  };
  ...
  routes.forEach((route) => {
    ...
    assertOpeningDeckhandCounterCheck(!drillDesc.includes('Opening Prep'), `${route.label} Cache Drill previewed cache-granted Opening Prep: ${drillDesc}`);
    ...
    assertOpeningDeckhandCounterCheck(!reward.openingCounterPrep, `${route.label} starter Cache Drill granted Opening Counter Prep`);
    assertOpeningDeckhandCounterCheck(G.openingCounterPlan === false, `${route.label} G.openingCounterPlan set by starter Cache Drill`);
    ...
    updateOpeningCounterPlanForSim(scene, G);
    assertOpeningDeckhandCounterCheck(G.openingCounterPlan === false, `${route.label} full-send completion armed Opening Prep`);
    results.push({ name: `${route.label} ${starterName(route.starterType)} can claim Boarding 1 Cache Drill without Opening Prep on full send`, ok: true, drillDesc });
  });

  routes.forEach((route) => {
    const { G, pirate } = setupCache(route, route.starterType, { round: 2 });
    const reward = applyScoutedCacheDrillForSim(scene, pirate);
    assertOpeningDeckhandCounterCheck(reward && reward.applied && !reward.openingCounterPrep, `${route.label} setup granted starter cache prep`);
    ...
    results.push({ name: `${route.label} route primary spends one-short Opening Prep, takes the starter Cache Drill bounty mark, defaults front-left, and pays doubled Ambush Bounty`, ok: true });
  });

  routes.forEach((route) => {
    const { G, pirate } = setupCache(route, route.starterType, { round: 2 });
    const reward = applyScoutedCacheDrillForSim(scene, pirate);
    assertOpeningDeckhandCounterCheck(reward && reward.applied && !reward.openingCounterPrep, `${route.label} commitment setup granted starter cache prep`);
    G.phase = 'shopping';
    G.busy = false;
    G.shopAnimating = false;
    G.shopCreditUsed = false;
    G.fullCrewDiscount = 0;
    G.openingCounterPlan = false;
    G.openingRoutePrimaryCommitmentMainKey = route.mainKey;
    G.enthusiasm = api.TYPES[route.primary].cost;
    G.shop = api.normalizeOpeningRouteShop(
      [route.primary, 'drummer', 'herald', 'trainer'],
      G.round,
    ...
    const quote = scene.shopPurchaseQuote(route.primary);
    assertOpeningDeckhandCounterCheck(
      quote.canBuy && quote.counter && quote.topDeck && quote.openingRoutePrimaryCommitment && !quote.openingCounterPrepMight && quote.discount === 0 && quote.fullCrewCoverage === 0,
      `${route.label} commitment primary quote mismatch: ${JSON.stringify(quote)}`
    );
    const bought = scene.buyPirate(G.shop.indexOf(route.primary), { deferRender: true, silent: true, ignoreAnimating: true, skipPanelRefresh: true });
    assertOpeningDeckhandCounterCheck(bought && bought.type === route.primary, `${route.label} commitment primary buy failed`);
    assertOpeningDeckhandCounterCheck(G.deck[G.deck.length - 1] === bought && !G.discard.includes(bought), `${route.label} commitment primary did not top-deck`);
    assertOpeningDeckhandCounterCheck((G.counterWatchIds || []).includes(bought.id), `${route.label} commitment primary did not gain Counter Watch`);
    assertOpeningDeckhandCounterCheck((bought.might || 0) === 0 && !bought.weaponKey && (bought.tempo || 0) === 0, `${route.label} commitment primary gained prep/prepared stats: ${JSON.stringify(bought)}`);
    assertOpeningDeckhandCounterCheck((pirate.might || 0) === 1, `${route.label} starter lost Cache Drill Might on commitment pass-off`);
    assertOpeningDeckhandCounterCheck((G.cacheDrillMusterIds || []).includes(pirate.id), `${route.label} commitment pass-off moved starter early report`);
    assertOpeningDeckhandCounterCheck(G.openingRouteCounterBoughtMainKey === route.mainKey && G.openingRouteCounterBoughtPirateId === bought.id, `${route.label} commitment primary did not secure route`);
    assertOpeningDeckhandCounterCheck((G.cacheDrillBountyMarks || []).length === 1 && G.cacheDrillBountyMarks[0].pirateId === bought.id && G.cacheDrillBountyMarks[0].mainKey === route.mainKey, `${route.label} commitment primary did not take starter bounty mark: ${JSON.stringify(G.cacheDrillBountyMarks)}`);
    assertBoughtPrimaryDoubledBountyAfterPassOff(G, route, pirate, bought, `${route.label} commitment pass-off`);
    results.push({ name: `${route.label} Route Primary Commitment passes the starter Cache Drill bounty mark to the bought primary`, ok: true, quote });
  });
...

  {
    ...
    const reward = applyScoutedCacheDrillForSim(scene, pirate);
    assertOpeningDeckhandCounterCheck(reward && reward.applied, 'commitment-only secured primary setup did not drill');
    assertOpeningDeckhandCounterCheck(!reward.securedRouteCachePassOff, 'commitment-only primary received secured Cache Drill pass-off');
    assertOpeningDeckhandCounterCheck((G.cacheDrillBountyMarks || [])[0].pirateId === pirate.id, 'commitment-only primary took starter bounty mark');
    results.push({ name: 'commitment-only route primaries do not receive Secured Route Cache Pass-Off or doubled Ambush Bounty marks', ok: true });
  }

  {
    ...
    G.openingRouteCounterBoughtPirateId = null;
    const reward = applyScoutedCacheDrillForSim(scene, pirate);
    assertOpeningDeckhandCounterCheck(reward && reward.applied && !reward.openingCounterPrep, 'discard-only prior primary cache drill granted prep');
    ...
    results.push({ name: 'discard-only route-primary investment does not block later one-short prep or Route Starter Pass-Off', ok: true, quote });
  }

  {
    ...
    const { G, pirate } = setupCache(route, route.starterType, { round: 2 });
    const reward = applyScoutedCacheDrillForSim(scene, pirate);
    assertOpeningDeckhandCounterCheck(reward && reward.applied && !reward.openingCounterPrep, 'negative setup granted starter cache prep');
    G.openingCounterPlan = true;
    G.phase = 'shopping';
    G.busy = false;
    G.shopAnimating = false;
    ...
    assertOpeningDeckhandCounterCheck(G.openingCounterPlan === false, 'side-prep buy did not consume one-short prep');
    ...
    results.push({ name: 'Opening Side Prep spends one-short prep without triggering Route Starter Pass-Off', ok: true });
  }

  {
    ...
    const { G, pirate } = setupCache(route, route.starterType, { round: 2 });
    const reward = applyScoutedCacheDrillForSim(scene, pirate);
    assertOpeningDeckhandCounterCheck(reward && reward.applied && !reward.openingCounterPrep, 'Full Crew negative setup granted starter cache prep');
    G.phase = 'shopping';
    G.busy = false;
    G.shopAnimating = false;
    G.shopCreditUsed = false;
    ...
    assertOpeningDeckhandCounterCheck((bought.might || 0) === 0 && !bought.weaponKey && (bought.tempo || 0) === 0, `Full Crew-only primary gained prep/prepared stats: ${JSON.stringify(bought)}`);
    assertOpeningDeckhandCounterCheck((G.cacheDrillBountyMarks || [])[0].pirateId === bought.id, 'Full Crew-only primary did not take the starter bounty mark');
    assertOpeningDeckhandCounterCheck((pirate.might || 0) === 1 && (G.cacheDrillMusterIds || []).includes(pirate.id), 'starter lost Might or early report during Full Crew pass-off');
    assertBoughtPrimaryDoubledBountyAfterPassOff(G, route, pirate, bought, 'Full Crew-only pass-off');
    results.push({ name: 'Full Crew route-primary buys pass the starter Cache Drill bounty mark without prep stats', ok: true });
  }

  {
    const route = routes[1];
    const { G, pirate } = setupCache(route, route.starterType, { round: 1 });
    const reward = applyScoutedCacheDrillForSim(scene, pirate);
    assertOpeningDeckhandCounterCheck(reward && reward.applied && !reward.openingCounterPrep, 'Full Crew coverage setup granted starter cache prep');
    G.phase = 'shopping';
    G.busy = false;
    G.shopAnimating = false;
    G.shopCreditUsed = false;
    G.fullCrewDiscount = 1;
    G.openingCounterPlan = false;
    G.enthusiasm = Math.max(0, api.TYPES[route.primary].cost - 2);
    G.shop = [route.primary, 'drummer', 'herald', 'trainer'];
    const quote = scene.shopPurchaseQuote(route.primary);
    assertOpeningDeckhandCounterCheck(quote.canBuy && quote.topDeck && quote.fullCrewCoverage === 1 && !quote.openingCounterPrepMight, `Full Crew coverage route primary quote mismatch: ${JSON.stringify(quote)}`);
    const bought = scene.buyPirate(0, { deferRender: true, silent: true, ignoreAnimating: true, skipPanelRefresh: true });
    assertOpeningDeckhandCounterCheck(bought && bought.type === route.primary, 'Full Crew coverage primary buy failed');
    assertOpeningDeckhandCounterCheck((bought.might || 0) === 0 && !bought.weaponKey && (bought.tempo || 0) === 0, `Full Crew coverage primary gained prep/prepared stats: ${JSON.stringify(bought)}`);
    assertOpeningDeckhandCounterCheck((G.cacheDrillBountyMarks || [])[0].pirateId === bought.id, 'Full Crew coverage primary did not take the starter bounty mark');
    assertOpeningDeckhandCounterCheck((pirate.might || 0) === 1 && (G.cacheDrillMusterIds || []).includes(pirate.id), 'starter lost Might or early report during Full Crew coverage pass-off');
    assertBoughtPrimaryDoubledBountyAfterPassOff(G, route, pirate, bought, 'Full Crew coverage pass-off');
    results.push({ name: 'Full Crew coverage route-primary buys pass the starter Cache Drill bounty mark without prep stats', ok: true });
  }

  {
    ...
    const { G, pirate } = setupCache(route, route.starterType, { round: 2 });
    const reward = applyScoutedCacheDrillForSim(scene, pirate);
    assertOpeningDeckhandCounterCheck(reward && reward.applied && !reward.openingCounterPrep, 'wrong-main negative setup granted starter cache prep');
...

  {
    ...
    const { G, pirate } = setupCache(route, route.starterType, { round: 2 });
    const reward = applyScoutedCacheDrillForSim(scene, pirate);
    assertOpeningDeckhandCounterCheck(reward && reward.applied && !reward.openingCounterPrep, 'removed-starter negative setup granted starter cache prep');
    G.openingCounterPlan = true;
    removePirateById(G, pirate.id);
    G.phase = 'shopping';
    G.busy = false;
...

  {
    ...
    assertOpeningDeckhandCounterCheck(G.openingCounterPlan === false, 'nonmatching starter armed Opening Prep from cache');
    ...
    const quote = scene.shopPurchaseQuote(route.primary);
    assertOpeningDeckhandCounterCheck(
      quote.canBuy && !quote.topDeck && !quote.openingCounterPrepMight && !quote.alarmRushedRouteCounter,
      `nonmatching cache opener exposed route primary setup perks: ${JSON.stringify(quote)}`
    );
    results.push({ name: 'nonmatching first starter opens Boarding 1 cache without Opening Prep or Cache Drill', ok: true, quote });
  }
...

  {
    ...
    assertOpeningDeckhandCounterCheck(!reward.openingCounterPrep && G.openingCounterPlan === false, 'shop counter opener received cache-granted Opening Prep');
    ...
    results.push({ name: 'bought shop-counter cache openers keep normal Cache Drill without Opening Prep', ok: true });
  }
...

  {
    ...
    scene._sacrificedIds.add(pirate.id);
    const reward = applyScoutedCacheDrillForSim(scene, pirate);
    assertOpeningDeckhandCounterCheck(!reward && G.openingCounterPlan === false, 'removed starter opener gained cache prep without Cache Drill');
    ...
    results.push({ name: 'removed route starter openers claim cache without Opening Prep or Cache Drill', ok: true });
  }
...
function runOpeningRouteMusterChecks(runtime) {
  ...
  routes.forEach((route) => {
    ...
    assertOpeningRouteMusterCheck(G.openingCounterPlan === false, `${route.label} full-send starter Cache Drill armed Opening Counter Prep`);
    assertOpeningRouteMusterCheck(quote.canBuy && !quote.credit && quote.topDeck && !quote.openingCounterPrepMight, `${route.label} route primary not affordable as Full Crew top-deck counter without prep: ${JSON.stringify(quote)}`);
    const bought = scene.buyPirate(G.shop.indexOf(route.primary), { deferRender: true, silent: true, ignoreAnimating: true, skipPanelRefresh: true });
    ...
    assertOpeningRouteMusterCheck((bought.might || 0) === 0 && !bought.weaponKey && (bought.tempo || 0) === 0, `${route.label} Full Crew route primary gained prep/prepared stats`);
    assertOpeningRouteMusterCheck((G.cacheDrillBountyMarks || []).some(mark => mark && mark.pirateId === bought.id && mark.mainKey === route.mainKey), `${route.label} Full Crew route primary did not receive starter bounty mark`);
    assertOpeningRouteMusterCheck((starter.might || 0) === 1 && (G.cacheDrillMusterIds || []).includes(starter.id), `${route.label} starter lost Cache Drill Might or early report after Full Crew pass-off`);
    ...
    results.push({ name: `${route.label} mustered-starter full-send affords a watched route-primary counter and passes the starter bounty mark`, ok: true, quote });
  });
...
function runCounterRecruitsReportEarlyChecks(runtime) {
...

  {
    ...
    const fullLine = scene.formatSendingPlanLine(scene.sendingPlanProjection(2, { includePortDrill: true }));
    assertCounterRecruitsReportEarlyCheck(endLine.includes('route commitment') && endLine.includes('top deck') && !endLine.includes('Opening Prep') && !endLine.includes('prepared') && !endLine.includes('Cover'), `End-now cash plan should show commitment top-deck without prep/Cover: ${endLine}`);
    ...
    results.push({ name: 'sending plan separates route commitment, Opening Prep setup, and Full Crew watched counters before Boarding 1', ok: true, endLine, partialLine, fullLine });
  }
...
function runHeuristicSendingAndShipPhase(runtime, api, scene) {
  ...
  if (scene && typeof scene.enterShoppingPhase === 'function') {
    scene.enterShoppingPhase();
  } else {
    G.phase = 'shopping';
    G.shopCreditUsed = false;
...
async function runShoppingPhase(
  ...
  while (true) {
    ...
    if (boughtType) {
      ...
      if (quote && quote.credit) purchases.push(`${label} (Credit +${quote.alert} Alert${coverText})`);
...
async function main() {
  ...
  if (opts.checkNoAlarmRushRouteCounter) {
    const result = runNoAlarmRushRouteCounterChecks(runtime);
    process.stdout.write(JSON.stringify(result, null, 2) + '\n');
    return;
  }

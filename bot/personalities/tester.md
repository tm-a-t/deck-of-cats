...
2. **Follow the Playwright CLI guide** in `bot/docs/codex-playwright-validation-guide.md` exactly and without improvisation.
...
Use the deterministic Playwright flow from `bot/docs/codex-playwright-validation-guide.md`.

Core rules:
1. Run Playwright only when the task touches browser-visible behavior.
2. Prefer the wrapper script referenced in the guide.
3. Start a local server in the prescribed deterministic way.
4. Always snapshot before using element references.
5. Keep checks short and goal-oriented.
6. Always clean up server and browser sessions.
